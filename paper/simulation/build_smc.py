...
def bi_level_SMC(data: tuple, 
    ...
    ** SMC algorithm: **
    ...
    prior = distributions.BiLevelPrior(p_group=p_group, 
                                       p_ind=p_ind, 
                                       pi_group=pi_group, 
                                       pi_ind=pi_ind, 
                                       dict_group=data[2])
    ...
    if approximation_method == 'ALA':
            model = binary_smc.BilevelALA(data=data, 
                                          prior=prior)
    elif approximation_method == 'LA':
         model = binary_smc.BilevelLA(data=data, 
                                      prior=prior)
    
    # Step 4: Specify the MCMC algorithm
    mcmc = smc_samplers.MCMCSequenceWF(mcmc=binary_smc.BiLevelBinaryMetropolis(data=data), 
                                       len_chain=P)
    ...
    selection = particles.multiSMC(fk=smc, 
                                   N=N // P, 
                                   nruns=nruns, 
                                   nprocs=nprocs, 
                                   verbose=0)
