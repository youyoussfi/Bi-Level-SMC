...
from build_particles import distributions as dists, smc_samplers as ssps
...
class BIC(VariableSelection):
    ...
    def loglik(self, gamma, t=None):
        ...
        l = -(self.coef_len * len_gam + self.coef_log * np.log(self.coef_in_log - wtw))
        return l
...
class BayesianLA(VariableSelection):
    ...
    def loglik(self, gamma, t=None):
        ...
        for n in range(N):
            ...
            beta_hat = optimize.minimize(
                fun=lambda z: -np.array(
                    probit.loglike(params=z) - np.sum(norm.logpdf(z))
                ),
                jac=lambda z: -probit.score(params=z) + z,
                hess=lambda z: -probit.hessian(params=z)
                + np.diag(np.repeat(1, len(z))),
                x0=np.repeat(0, xg.shape[1]),
                method="Newton-CG",
            )["x"]

            la[n] = (
                probit.loglike(params=beta_hat)
...
class BilevelLA(VariableSelectionBiLevel):
    ...
    def loglik(self, gamma, t=None):
        ...
        for n in range(N):
            ...
            beta_hat = optimize.minimize(
                fun=lambda z: -np.array(
                    probit.loglike(params=z) - np.sum(norm.logpdf(z, loc=0, scale=5))
                ),
                jac=lambda z: -probit.score(params=z) + z,
                hess=lambda z: -probit.hessian(params=z)
                + np.diag(np.repeat(1, len(z))),
...
class BayesianALA(VariableSelection):
    ...
    def loglik(self, gamma, t=None):
        ...
        for n in range(N):
            ...
            beta_hat = beta_0 - reduce(
                np.dot, [inv_matrix, probit.score(params=beta_0)]
            )
...
class BiLevelProposal(dists.DiscreteDist):
    ...
    def fit(cls, W, x, dict_group, probs_thresh=0.02, corr_thresh=0.075):
        ...
        for i in range(dim - p_group):
            if np.sum(ws[:, i] > 0):
                ph[p_group + i] = np.average(
                    xs[:, p_group + i], weights=ws[:, i], axis=0
                )
            else:
                ph[p_group + i] = 0
